...
class TriangleTest {
    ...
    void testFindIntersections() {
        ...
        //אם מוחזר null
        assertEquals(triangle.findIntersections(ray).size(), 1, "ERROR: Wrong number of points intersects the triangle " +
        "when the point Inside the triangle");

        assertEquals(triangle.findIntersections(ray).get(0), new Point(1,2,2), "ERROR: Wrong number of points intersects the triangle " +
        "when the point Inside the triangle");

        //TC02: the point outside against edge
        ray = new Ray(new Point(2,0,0), new Vector(-4, 1,2));
        //אם מוחזר null
        assertNull(triangle.findIntersections(ray), "ERROR: Wrong number of points intersects the triangle " +
